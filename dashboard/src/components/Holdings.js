...
const Holdings = () => {
  ...
  return (
    <>
      ...
      <DataCardList emptyMessage="No holdings yet.">
        {allHoldings.map((stock, index) => {
          const currValue = stock.price * stock.qty;
          ...
          return (
            <DataCard
              ...
              rows={[
                { label: "Avg. cost", value: stock.avg.toFixed(2) },
                { label: "LTP", value: stock.price.toFixed(2) },
                { label: "Current value", value: currValue.toFixed(2) },
                { label: "Net chg.", value: stock.net, valueClass: profClass },
                { label: "Day chg.", value: stock.day, valueClass: dayClass },
              ]}
            />
          );
        })}
